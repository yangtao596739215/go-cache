...
//把业务的producer转成内部使用的producer
func retryWrapper(producer BusinessProducer, reTryTimes int) PendingProducer {
	fn := func(res chan interface{}, errChan chan error) {
		var err error
		for reTryTimes >= 0 {
			v, err := producer()
...
func (pCache *PendingCache) Get(L ILogger, key string, producer BusinessProducer) (interface{}, error) {
	entity, isHit, err := pCache.get(L, key, commonWrapper(producer))
...
func (pCache *PendingCache) GetWithRetry(L ILogger, key string, producer BusinessProducer, retryTimes int) (interface{}, error) {
	entity, isHit, err := pCache.get(L, key, retryWrapper(producer, retryTimes))
	return handleResult(L, key, entity, isHit, err)
}
...
func handleResult(L ILogger, key string, entity *Entity, isHit bool, err error) (interface{}, error) {
	...
	//打印日志，方便统计是否命中
	if isHit {
		L.INFO("get pending_cache_hit key:%d", key)
	} else {
		L.INFO("get pending_cache_not_hit roomId:%d", key)
	}
